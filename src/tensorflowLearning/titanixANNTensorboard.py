...
def addALayer(inputSize, outputSize, input, activationFuction=None):
    with tf.name_scope("layer"):
        with tf.name_scope("W"):
            Weight = tf.Variable(tf.random_normal([inputSize, outputSize]), name="Weight")
        with tf.name_scope("B"):
            Bias = tf.Variable(tf.random_normal([1, outputSize])+ 0.1, name="bias")
...
with tf.name_scope("onputs"):
    x = tf.placeholder("float", shape=[None, featureNum], name="input_data")
    y = tf.placeholder("float", shape=[None, 1], name="output_data")

outputLayer1 = addALayer(featureNum, 100, x, tf.nn.relu)
outputLayer_1 = addALayer(100, 100, outputLayer1, tf.nn.relu)
outputLayer_2 = addALayer(100, 100, outputLayer_1, tf.nn.relu)
